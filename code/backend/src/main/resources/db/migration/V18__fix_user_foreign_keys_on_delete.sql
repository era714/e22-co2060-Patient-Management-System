...
BEGIN
    ...
    -- 2. management_activity_log: Set performed_by / target_user to NULL
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'management_activity_log') THEN
        ALTER TABLE management_activity_log DROP CONSTRAINT IF EXISTS fk_management_log_performed_by;
        ALTER TABLE management_activity_log ADD CONSTRAINT fk_management_log_performed_by 
            FOREIGN KEY (performed_by_id) REFERENCES users(id) ON DELETE SET NULL;

        ALTER TABLE management_activity_log DROP CONSTRAINT IF EXISTS fk_management_log_target_user;
        ALTER TABLE management_activity_log ADD CONSTRAINT fk_management_log_target_user 
            FOREIGN KEY (target_user_id) REFERENCES users(id) ON DELETE SET NULL;
    END IF;

    -- 3. profile_change_requests: Cascade delete requests when user is deleted, set reviewer to NULL
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'profile_change_requests') THEN
