      ...
      <aside className={`w-[320px] bg-white border-r border-slate-200 flex flex-col transition-all duration-300 z-20 ${isSidebarOpen ? "translate-x-0" : "-translate-x-full fixed h-full shadow-2xl"}`}>
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-950">
          <div className="bg-slate-800 rounded-xl p-3 flex items-center gap-3 mb-2">
            <div className={`w-9 h-9 rounded-full ${ACCENT.iconBg} flex items-center justify-center text-teal-700 font-bold border ${ACCENT.border}`}>
              {user?.email?.charAt(0).toUpperCase() || "N"}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-white truncate">Nurse {user?.firstName || ""}</p>
              <p className="text-xs text-slate-400 truncate">{user?.email}</p>
            </div>
          </div>
          <button
            onClick={handleLogout}
            className="w-full flex items-center justify-center gap-2 px-3 py-2 text-xs text-red-400 hover:text-red-300 hover:bg-red-400/10 rounded-lg transition-colors"
          >
